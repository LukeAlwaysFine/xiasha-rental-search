...
| 后端 | FastAPI + APScheduler | 速率限制，6h poster 检测，1h 中介重评 |
| 前端 | Vanilla JS + 高德 JS API 2.0 | taste-skill 设计体系，暗色模式 |

## 数据流

```
搜索 → 抓取（~165关键词）→ LLM 提取（40并发）→ API坐标优先
→ geocode兜底 → LLM+Regex 中介检测（默认"疑似中介"）
→ 入库 → poster主页检测（5并发~6min）→ LLM重排/去重 → 前端
```

## 关键决策

- **poster_id**: MTOP `userNickName` 提取，UPSERT COALESCE 回填。MTOP 首响应 dump exContent keys 便于排查
- **中介判定**: LLM 仅输出"中介"/"疑似中介"，"个人"唯一来源是 Playwright 进主页核实（1出租+有其他物品）
- **Poster 检查**: 5 并发 tab（Semaphore + asyncio.gather），~6min 完成 500+，判定四档（≥3→中介，2→疑似中介，1+无其他→疑似中介，1+有其他→个人）
